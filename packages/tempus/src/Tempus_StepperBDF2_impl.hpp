  ...
  Teuchos::RCP<Teuchos::ParameterList> const& pList)
{
  ...
  if (!(this->stepperPL_->isParameter("Solver Name"))) {
    this->stepperPL_->template set<std::string>("Solver Name", "Default Solver");
    Teuchos::RCP<Teuchos::ParameterList> solverPL = defaultSolverParameters();
    this->stepperPL_->template set("Default Solver", *solverPL);
  }
